...
export default function Toolbar({ toolbarMode, setToolbarMode, setBrushSize, assetList, setSaveToBackend, overwrite, setOverwrite }) {
                ...
                return (
                    <>
                        ...
                        <div className="flex flex-col gap-2 w-full p-2">
                            ...
                            <input type="checkbox" className="cursor-pointer" checked={overwrite} onChange={(e) => setOverwrite(e.target.checked)} />
                        </div>
                                ...
                                <div key={group} className="w-full flex flex-col items-center gap-2">
                                    ...
                                    {Array.isArray(files) && files.map((filename) => (
                                        <img
                                            ...
                                            onClick={() => {
                                                setToolbarMode(`${group}/${filename}`); console.log("Brush changed to: ", `${group}/${filename}`);
                                            }}
                                        />
                                    ))}
                                </div>
                ...
                return (
                    <div className="flex flex-col gap-2">
                        {toolItems.Tools.map((tool) => {
                            const isActive = toolbarMode === tool.toolName;
                            return (
                                <Button
                                    key={tool.id}
                                    title={tool.name}
    ...
    return (
        <div className={`h-screen bg-neutral-200 transition-all duration-300 border-l border-neutral-300 shadow-xl ${toolbarMode === "main" ? "w-20" : "w-72"
            }`}>
